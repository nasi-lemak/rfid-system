      ...
      <div className="grid cols-2">
        {data?.map((t) => (
          <div className="panel" key={t.id}>
            <div className="topbar"><div><h2 style={{ marginBottom: 2 }}>{t.name} <span className="muted small">{t.code}</span></h2><span className="muted small">{t.vertical ?? ''} · <Link to={`/items?itemTypeId=${t.id}`}>{t.itemCount ?? 0} items</Link></span></div><div className="row"><button className="sm" onClick={() => open(t)}>Edit</button><button className="sm danger" onClick={() => confirm('Delete type?') && remove.mutate(t.id)}>Delete</button></div></div>
