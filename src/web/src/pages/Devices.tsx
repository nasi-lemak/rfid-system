      ...
      {edit && <Modal title={edit.id ? 'Edit device' : 'New device'} onClose={() => setEdit(null)} width={720}>
        <form className="form" onSubmit={submit}>
          <div className="form cols-2">
            ...
            <label className="row" style={{ alignItems: 'center' }}><input type="checkbox" style={{ width: 'auto' }} checked={String(edit.config?.edgeManaged) === 'true'} onChange={(e) => setEdit({ ...edit, config: { ...edit.config, edgeManaged: e.target.checked ? true : undefined } })} /> Driven by an on-site edge agent (the server will not open its own LLRP connection)</label>
            {String(edit.config?.edgeManaged) === 'true' && <label>Edge gateway (the agent pulls this reader's configuration)<select value={String(edit.config?.edgeGatewayId ?? '')} onChange={(e) => setEdit({ ...edit, config: { ...edit.config, edgeGatewayId: e.target.value || undefined } })}><option value="">— not assigned (agent uses its local config) —</option>{data?.filter((g) => g.kind === 'Gateway').map((g) => <option key={g.id} value={g.id}>{g.name}</option>)}</select></label>}</>}
