    ...
    <div>
      ...
      <div className="grid cols-2">
        {data?.map((d) => <div className="panel" key={d.id}>
          ...
          <table><thead><tr><th>Port</th><th>Location</th><th>Direction</th><th>Power</th></tr></thead><tbody>{d.antennas.map((a) => <tr key={a.port}><td>{a.port}</td><td>{a.locationName ?? <span className="muted">unmapped</span>}</td><td>{a.direction !== 'None' ? <Badge tone={a.direction === 'In' ? 'ok' : 'warn'}>{a.direction}</Badge> : <span className="muted">presence</span>}</td><td>{a.powerDbm != null ? `${a.powerDbm} dBm` : ''}</td></tr>)}</tbody></table>
        ...
        <form className="form" onSubmit={submit}>
          ...
          <div><h3>Antennas</h3>
            {ants.map((a, i) => <div className="row" key={i} style={{ marginBottom: 6 }}>
              ...
              <input type="number" step="0.1" style={{ width: 90 }} placeholder="dBm" value={a.powerDbm ?? ''} onChange={(e) => setAnt(i, { powerDbm: e.target.value ? Number(e.target.value) : null })} />
