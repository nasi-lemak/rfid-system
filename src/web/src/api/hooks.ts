...
export const useStockSummary = () => useQuery({ queryKey: ['stock-summary'], queryFn: () => get<StockSummary[]>('/api/stock/summary'), refetchInterval: 30000 });
export const useStockBalances = (params: Record<string, unknown>) => useQuery({ queryKey: ['stock-balances', params], queryFn: () => get<StockBalance[]>('/api/stock/balances', params) });
export const useStockMovements = (params: Record<string, unknown>) => useQuery({ queryKey: ['stock-movements', params], queryFn: () => get<StockMovement[]>('/api/stock/movements', params) });
export const useWorkflows = () => useQuery({ queryKey: ['workflows'], queryFn: () => get<WorkflowDefinition[]>('/api/workflows', { includeDisabled: true }) });
export const useWorkflowRuns = () => useQuery({ queryKey: ['workflow-runs'], queryFn: () => get<WorkflowRunSummary[]>('/api/workflows/runs', { take: 100 }), refetchInterval: 15000 });
export const useOperationDefinitions = () => useQuery({ queryKey: ['operation-definitions'], queryFn: () => get<OperationDefinition[]>('/api/operations/definitions'), staleTime: 5 * 60_000 });
