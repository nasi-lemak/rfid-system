...
public class AnalyticsService
{
    ...
    public async Task<Series> TrendAsync(string metric, int days, string bucket = "day", DateTime? now = null, SiteScope? scope = null, CancellationToken ct = default)
    {
        var sc = scope ?? SiteScope.All; var t = now ?? DateTime.UtcNow; var since = t.Date.AddDays(-(days - 1));
        List<(DateTime day, int count)> rows = metric switch
        {
            "reads" => (await sc.Reads(_db.TagReads).Where(r => r.ReadAt >= since).GroupBy(r => r.ReadAt.Date).Select(g => new { g.Key, C = g.Count() }).ToListAsync(ct)).Select(x => (x.Key, x.C)).ToList(),
            "operations" => (await sc.Operations(_db.Operations).Where(o => o.StartedAt >= since).GroupBy(o => o.StartedAt.Date).Select(g => new { g.Key, C = g.Count() }).ToListAsync(ct)).Select(x => (x.Key, x.C)).ToList(),
            "events" => (await sc.Events(_db.ItemEvents).Where(e => e.OccurredAt >= since).GroupBy(e => e.OccurredAt.Date).Select(g => new { g.Key, C = g.Count() }).ToListAsync(ct)).Select(x => (x.Key, x.C)).ToList(),
            "alerts" => (await sc.Alerts(_db.Alerts).Where(a => a.RaisedAt >= since).GroupBy(a => a.RaisedAt.Date).Select(g => new { g.Key, C = g.Count() }).ToListAsync(ct)).Select(x => (x.Key, x.C)).ToList(),
            "moves" => (await sc.Events(_db.ItemEvents).Where(e => e.OccurredAt >= since && e.Type == ItemEventType.Moved).GroupBy(e => e.OccurredAt.Date).Select(g => new { g.Key, C = g.Count() }).ToListAsync(ct)).Select(x => (x.Key, x.C)).ToList(),
    ...
    public async Task<List<UtilizationRow>> UtilizationAsync(int days, DateTime? now = null, SiteScope? scope = null, CancellationToken ct = default)
    {
        var sc = scope ?? SiteScope.All; var t = now ?? DateTime.UtcNow; var since = t.AddDays(-days);
        var items = await sc.Items(_db.Items).Where(i => i.Status != ItemStatus.Disposed).Select(i => new { i.Id, i.ItemTypeId, i.Status, i.CustodianPartyId, i.CycleCount }).ToListAsync(ct);
        var types = await _db.ItemTypes.ToDictionaryAsync(x => x.Id, x => x.Name, ct);
        var touched = await sc.Events(_db.ItemEvents).Where(e => e.OccurredAt >= since).GroupBy(e => e.ItemId).Select(g => new { g.Key, Events = g.Count(), Moves = g.Count(e => e.Type == ItemEventType.Moved) }).ToDictionaryAsync(x => x.Key, ct);
    ...
    public async Task<List<DwellRow>> DwellAsync(int days, DateTime? now = null, SiteScope? scope = null, CancellationToken ct = default)
    {
        var sc = scope ?? SiteScope.All; var t = now ?? DateTime.UtcNow; var since = t.AddDays(-days);
        var sessions = await sc.Sessions(_db.PresenceSessions).Where(s => s.EnteredAt >= since).Select(s => new { s.LocationId, s.ItemId, s.EnteredAt, End = s.ExitedAt ?? s.LastSeenAt }).ToListAsync(ct);
    ...
    public async Task<AccuracyReport> AccuracyAsync(int days, DateTime? now = null, SiteScope? scope = null, CancellationToken ct = default)
    {
        var sc = scope ?? SiteScope.All; var t = now ?? DateTime.UtcNow; var since = t.AddDays(-days);
        var sts = await sc.Stocktakes(_db.Stocktakes).Where(s => (s.Status == StocktakeStatus.Reconciled || s.Status == StocktakeStatus.Applied) && (s.CompletedAt ?? s.StartedAt) >= since).OrderBy(s => s.CompletedAt ?? s.StartedAt).ToListAsync(ct);
    ...
    public async Task<List<AlertResponseRow>> AlertResponseAsync(int days, DateTime? now = null, SiteScope? scope = null, CancellationToken ct = default)
    {
        var sc = scope ?? SiteScope.All; var t = now ?? DateTime.UtcNow; var since = t.AddDays(-days);
        var alerts = await sc.Alerts(_db.Alerts).Where(a => a.RaisedAt >= since).Select(a => new { a.Severity, a.Status, a.RaisedAt, a.AcknowledgedAt, a.ClosedAt, a.EscalationLevel }).ToListAsync(ct);
