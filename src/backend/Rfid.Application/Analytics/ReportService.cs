...
public class ReportService
{
    ...
    public async Task<(List<string> columns, List<object?[]> rows)> RunAsync(string code, IDictionary<string, string?> p, SiteScope scope, CancellationToken ct = default)
    {
        ...
        switch (code)
        {
            case "inventory":
            {
                var rows = await scope.Items(_db.Items).Include(i => i.ItemType).Include(i => i.CurrentLocation).Include(i => i.CustodianParty).Where(i => i.Status != ItemStatus.Disposed).OrderBy(i => i.CurrentLocation!.Name).ThenBy(i => i.Name).ToListAsync(ct);
            ...
            case "missing":
            {
                ...
                var rows = await scope.Items(_db.Items).Include(i => i.ItemType).Include(i => i.CurrentLocation).Where(i => i.Status == ItemStatus.Missing || i.LastSeenAt == null || i.LastSeenAt < since).Where(i => i.Status != ItemStatus.Disposed).OrderBy(i => i.LastSeenAt).ToListAsync(ct);
            ...
            case "custody":
            {
                var rows = await scope.Items(_db.Items).Include(i => i.ItemType).Include(i => i.CustodianParty).Include(i => i.CurrentLocation).Where(i => i.CustodianPartyId != null && i.Status != ItemStatus.Disposed).OrderBy(i => i.CustodianParty!.Name).ToListAsync(ct);
            ...
            case "overdue":
            {
                var rows = await scope.Items(_db.Items).Include(i => i.ItemType).Include(i => i.CustodianParty).Where(i => i.DueBackAt != null && i.DueBackAt < now && i.Status != ItemStatus.Disposed).OrderBy(i => i.DueBackAt).ToListAsync(ct);
            ...
            case "expiry":
            {
                ...
                var rows = await scope.Items(_db.Items).Include(i => i.ItemType).Include(i => i.CurrentLocation).Where(i => i.ExpiryDate != null && i.ExpiryDate <= limit && i.Status != ItemStatus.Disposed).OrderBy(i => i.ExpiryDate).ToListAsync(ct);
            ...
            case "inspection":
            {
                ...
                var rows = await scope.Items(_db.Items).Include(i => i.ItemType).Include(i => i.CurrentLocation).Where(i => i.NextInspectionDue != null && i.NextInspectionDue <= limit && i.Status != ItemStatus.Disposed).OrderBy(i => i.NextInspectionDue).ToListAsync(ct);
            ...
            case "cycles":
            {
                var rows = await scope.Items(_db.Items).Include(i => i.ItemType).Where(i => i.ItemType!.TracksCycles && i.Status != ItemStatus.Disposed).OrderByDescending(i => i.CycleCount).ToListAsync(ct);
            ...
            case "depreciation":
            {
                var rows = await scope.Items(_db.Items).Include(i => i.ItemType).Include(i => i.CurrentLocation).Include(i => i.CustodianParty).Where(i => i.Cost != null).OrderBy(i => i.ItemType!.Name).ThenBy(i => i.Identifier).ToListAsync(ct);
            ...
            case "movements":
            {
                ...
                var q = scope.Events(_db.ItemEvents).Where(e => e.OccurredAt >= from && e.OccurredAt <= to);
            ...
            case "stocktakes":
            {
                var sts = await scope.Stocktakes(_db.Stocktakes).OrderByDescending(s => s.StartedAt).Take(500).ToListAsync(ct);
                var locs = await _db.Locations.ToDictionaryAsync(l => l.Id, l => l.Name, ct);
            ...
            case "reads":
            {
                ...
                var rows = await scope.Reads(_db.TagReads).Where(r => r.ReadAt >= since).GroupBy(r => new { r.DeviceId, Day = r.ReadAt.Date }).Select(g => new { g.Key.DeviceId, g.Key.Day, Count = g.Count(), Unique = g.Select(r => r.Epc).Distinct().Count() }).OrderBy(x => x.Day).ToListAsync(ct);
            ...
            {
                var since = now.AddDays(-Days(7));
                var sessions = await scope.Sessions(_db.PresenceSessions).Where(s => s.EnteredAt >= since).OrderByDescending(s => s.EnteredAt).Take(5000).ToListAsync(ct);
            ...
            case "print-jobs":
            {
                ...
                var jobs = await scope.PrintJobs(_db.PrintJobs).Where(j => j.RequestedAt >= since).OrderByDescending(j => j.RequestedAt).Take(5000).ToListAsync(ct);
            ...
            {
                var since = now.AddDays(-Days(30));
                var events = await scope.Events(_db.ItemEvents).Where(e => e.Type == ItemEventType.Imported && e.OccurredAt >= since).OrderByDescending(e => e.OccurredAt).Take(5000).ToListAsync(ct);
                var itemIds = events.Select(e => e.ItemId).Distinct().ToList();
                var items = await _db.Items.Where(i => itemIds.Contains(i.Id)).ToDictionaryAsync(i => i.Id, ct);
