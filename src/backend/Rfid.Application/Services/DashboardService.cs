...
public class DashboardService
{
    ...
    public async Task<List<WidgetResult>> EvaluateAsync(IEnumerable<DashboardWidget> widgets, CancellationToken ct = default)
    {
        var results = new List<WidgetResult>();
        foreach (var w in widgets)
        {
            try { results.Add(new WidgetResult(w.Id, w.Type, await EvaluateOneAsync(w, ct))); }
    ...
    private async Task<IQueryable<Item>> ScopedItemsAsync(DashboardWidget w, CancellationToken ct)
    {
        var q = _db.Items.Where(i => i.Status != ItemStatus.Disposed);
    ...
    private async Task<object?> EvaluateOneAsync(DashboardWidget w, CancellationToken ct)
    {
        ...
        switch (w.Type)
        {
            case "stat":
            {
                var items = await ScopedItemsAsync(w, ct);
                ...
                long value = filter switch
                {
                    ...
                    "openAlerts" => await _db.Alerts.CountAsync(a => a.Status == AlertStatus.Open, ct),
                    "criticalAlerts" => await _db.Alerts.CountAsync(a => a.Status == AlertStatus.Open && a.Severity == Severity.Critical, ct),
                    ...
                    "readsToday" => await _db.TagReads.CountAsync(r => r.ReadAt >= now.Date, ct),
                    "operationsToday" => await _db.Operations.CountAsync(o => o.StartedAt >= now.Date, ct),
                    "openStocktakes" => await _db.Stocktakes.CountAsync(s => s.Status == StocktakeStatus.Open, ct),
                    "devicesOffline" => await _db.Devices.CountAsync(d => d.Health == DeviceHealth.Offline, ct),
                    "devicesOnline" => await _db.Devices.CountAsync(d => d.Health == DeviceHealth.Online, ct),
                    "present" => await _db.PresenceSessions.CountAsync(s => s.ExitedAt == null, ct),
            ...
            case "breakdown":
            {
                var items = await ScopedItemsAsync(w, ct); var take = Math.Clamp(Int(w, "take", 10), 1, 30);
                ...
                var rows = by switch
                {
                    ...
                    "status" => await _db.Items.GroupBy(i => i.Status).Select(g => new { label = g.Key.ToString(), count = g.Count() }).OrderByDescending(x => x.count).ToListAsync(ct),
                    ...
                    "severity" => await _db.Alerts.Where(a => a.Status == AlertStatus.Open).GroupBy(a => a.Severity).Select(g => new { label = g.Key.ToString(), count = g.Count() }).OrderByDescending(x => x.count).ToListAsync(ct),
                    "alertSource" => await _db.Alerts.Where(a => a.Status == AlertStatus.Open).GroupBy(a => a.Source ?? "rule").Select(g => new { label = g.Key, count = g.Count() }).OrderByDescending(x => x.count).ToListAsync(ct),
            ...
            case "trend":
            {
                ...
                {
                    "reads" => await _db.TagReads.Where(r => r.ReadAt >= since).GroupBy(r => r.ReadAt.Date).Select(g => new { day = g.Key, count = g.Count() }).OrderBy(x => x.day).ToListAsync(ct),
                    "operations" => await _db.Operations.Where(o => o.StartedAt >= since).GroupBy(o => o.StartedAt.Date).Select(g => new { day = g.Key, count = g.Count() }).OrderBy(x => x.day).ToListAsync(ct),
                    "events" => await _db.ItemEvents.Where(e => e.OccurredAt >= since).GroupBy(e => e.OccurredAt.Date).Select(g => new { day = g.Key, count = g.Count() }).OrderBy(x => x.day).ToListAsync(ct),
                    "alerts" => await _db.Alerts.Where(a => a.RaisedAt >= since).GroupBy(a => a.RaisedAt.Date).Select(g => new { day = g.Key, count = g.Count() }).OrderBy(x => x.day).ToListAsync(ct),
            ...
            case "presence":
            {
                ...
                var open = _db.PresenceSessions.Where(s => s.ExitedAt == null);
            ...
            case "devices":
            {
                var rows = await _db.Devices.GroupBy(d => d.Health).Select(g => new { health = g.Key.ToString(), count = g.Count() }).ToListAsync(ct);
                var offline = await _db.Devices.Where(d => d.Health == DeviceHealth.Offline || d.Health == DeviceHealth.Degraded).OrderBy(d => d.Name).Take(8).Select(d => new { d.Id, d.Name, health = d.Health.ToString(), d.LastHeartbeatAt, d.LastSeenAt }).ToListAsync(ct);
                return new { total = rows.Sum(r => r.count), byHealth = rows, attention = offline };
            }
            case "stocktakes":
            {
                var take = Math.Clamp(Int(w, "take", 6), 1, 20);
                var rows = await _db.Stocktakes.Where(s => s.Status != StocktakeStatus.Cancelled).OrderByDescending(s => s.StartedAt).Take(take).Select(s => new { s.Id, s.Name, status = s.Status.ToString(), s.ExpectedCount, s.FoundCount, s.MissingCount, s.UnexpectedCount, s.StartedAt }).ToListAsync(ct);
            ...
            case "list":
            {
                ...
                {
                    var evs = await _db.ItemEvents.OrderByDescending(e => e.OccurredAt).Take(take).ToListAsync(ct);
                ...
                var alerts = await _db.Alerts.Where(a => a.Status == AlertStatus.Open).OrderByDescending(a => a.RaisedAt).Take(take).Select(a => new { a.Id, a.ItemId, severity = a.Severity.ToString(), a.Message, a.RaisedAt, a.Source, a.EscalationLevel }).ToListAsync(ct);
                return alerts;
            }
            case "fences":
            {
