...
builder.Services.AddScoped<Rfid.Application.Platform.IOutboxHandler>(sp => new Rfid.Application.Platform.LiveOutboxHandler(sp.GetRequiredService<Rfid.Application.Platform.ILiveTransport>(), Rfid.Application.Platform.OutboxKinds.LiveEvent, "event"));
builder.Services.AddScoped<Rfid.Application.Platform.IOutboxHandler>(sp => new Rfid.Application.Platform.LiveOutboxHandler(sp.GetRequiredService<Rfid.Application.Platform.ILiveTransport>(), Rfid.Application.Platform.OutboxKinds.LiveAlert, "alert"));
builder.Services.AddScoped<Rfid.Application.Platform.IOutboxHandler, Rfid.Application.Platform.WebhookOutboxHandler>();
builder.Services.AddScoped<Rfid.Application.Platform.IOutboxHandler, Rfid.Application.Platform.NotificationOutboxHandler>();
builder.Services.AddScoped<Rfid.Application.Platform.IOutboxHandler, IntegrationOutboxHandler>();
builder.Services.AddScoped<Rfid.Application.Operations.IOperationRunner, Rfid.Api.Background.ScopedOperationRunner>();
builder.Services.AddScoped<Rfid.Application.Platform.IOutboxHandler, Rfid.Application.Operations.RunOperationOutboxHandler>();
builder.Services.AddScoped<Rfid.Application.Platform.OutboxDispatcher>(sp => new Rfid.Application.Platform.OutboxDispatcher(sp.GetRequiredService<IAppDb>(), sp.GetServices<Rfid.Application.Platform.IOutboxHandler>()));
...
builder.Services.AddHostedService<Rfid.Api.Background.RuleSchedulerService>();
...
builder.Services.AddScoped<MaintenanceService>(sp => new MaintenanceService(sp.GetRequiredService<IAppDb>(), sp.GetRequiredService<ICurrentContext>(), sp.GetRequiredService<NotificationService>()) { AlertRisk = cfg.GetValue("Maintenance:AlertRisk", 70.0) });
builder.Services.AddScoped<EpcisService>(sp => new EpcisService(sp.GetRequiredService<IAppDb>(), sp.GetRequiredService<ICurrentContext>(), sp.GetRequiredService<ReadIngestionService>(), sp.GetRequiredService<OperationProcessor>(), sp.GetRequiredService<Rfid.Application.Operations.OperationDefinitions>()) { BaseUrl = new[] { cfg["Epcis:BaseUrl"], cfg["Notifications:BaseUrl"], "https://rfid-platform.local" }.First(v => !string.IsNullOrWhiteSpace(v))! });
builder.Services.AddScoped<Rfid.Api.Controllers.BillingController>();
builder.Services.AddHostedService<Rfid.Api.Background.BillingAccrualService>();
builder.Services.AddHostedService<Rfid.Api.Background.MaintenanceForecastService>();
...
app.UseMiddleware<PortalScopeMiddleware>();
