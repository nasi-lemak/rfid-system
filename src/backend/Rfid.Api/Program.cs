...
builder.Services.AddScoped<EpcisService>(sp => new EpcisService(sp.GetRequiredService<IAppDb>(), sp.GetRequiredService<ICurrentContext>(), sp.GetRequiredService<ReadIngestionService>(), sp.GetRequiredService<OperationProcessor>()) { BaseUrl = new[] { cfg["Epcis:BaseUrl"], cfg["Notifications:BaseUrl"], "https://rfid-platform.local" }.First(v => !string.IsNullOrWhiteSpace(v))! });
